...
public partial class PlayzorEditor : MudExBaseComponent<PlayzorEditor>
{
    ...
    /// </summary>
    [Parameter]
    public string PopoutUrl { get; set; } = "_content/Playzor.Blazor/popout.html";
    ...
    /// <summary>Names offered by the samples button. Empty hides the button.</summary>
    [Parameter]
    public IEnumerable<string> Samples { get; set; }

    /// <summary>Raised with the picked sample name. Loading it is up to the host.</summary>
    [Parameter]
    ...
    {
        CollectAllEditorContent();
        await OnSaveRequested.InvokeAsync(CodeFiles.Values.ToList());
    }
    ...
    private async Task ShowSamplesAsync()
    {
        ...
        var res = await DialogService.ShowComponentInDialogAsync<MudExList<string>>(L["Samples"], L["Select sample to open"],
            list =>
            {
                ...
                list.ItemCollection = Samples.Select(s => s.Replace("_", " ")).ToArray();
                list.Clickable = true;
                list.SearchBox = true;
    ...
    private Task SelectSampleAsync(string sample)
        => OnSampleSelected.InvokeAsync(sample?.Replace(" ", "_"));
