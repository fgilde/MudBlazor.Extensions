class MudExAppLoader extends HTMLElement{css=`
    ...
    `;constructor(){super();this.loadInlineStyles(this.css)}loadInlineStyles(n){try{const t=document.createElement("style");t.type="text/css";t.appendChild(document.createTextNode(n));document.head.appendChild(t)}catch(t){}}loadStylesheet(n){try{const t=document.createElement("link");t.rel="stylesheet";t.href=n;t.onerror=()=>{this.removeLoadingContainer()};document.head.appendChild(t)}catch(t){}}connectedCallback(){this.setAttributes();this.setNewMarkup();this.addPercentageObserver();this.addContentChangeObserver()}setAttributes(){this.accentColor=this.getAttribute("AccentColor")||"#ff0000";this.loadingTextColor=this.getAttribute("LoadingTextColor")||this.accentColor;this.appNameColor=this.getAttribute("AppNameColor")||this.accentColor;this.mainAppId=this.getAttribute("MainAppId")||this.getAttribute("AppId")||"app";this.logo=this.getAttribute("Logo")||"";this.timeout=parseInt(this.getAttribute("Timeout"))||2e3;this.preLoadText=this.getAttribute("PreLoadText")||"Loading...";this.appName=this.getAttribute("AppName")||"";document.documentElement.style.setProperty("--loading-color",this.accentColor);document.documentElement.style.setProperty("--loading-text-color",this.loadingTextColor);document.documentElement.style.setProperty("--app-name-text-color",this.appNameColor)}setNewMarkup(){const n=`
        ...
        `;this.innerHTML=n}addPercentageObserver(){const n=(n,t)=>{const i=Math.round(n);i!==this.prevPercentage&&(this.prevPercentage=i,requestAnimationFrame(()=>this.updateUI(i)));n>=100&&t.disconnect()};this.observeCSSVariable("--blazor-load-percentage",n)}addContentChangeObserver(){document.addEventListener("DOMContentLoaded",()=>{const n=document.getElementById(this.mainAppId);n?this.observeContentChanges(n,n=>{const t=this.querySelector(".mud-ex-app-loader-loading-container");t.classList.add("pixelated-out");n.disconnect();setTimeout(()=>{t.remove()},this.timeout||2e3)}):this.removeLoadingContainer(this.timeout||2e3)})}removeLoadingContainer(n){setTimeout(()=>{const n=this.querySelector(".mud-ex-app-loader-loading-container");n&&n.remove()},n||1)}observeCSSVariable(n,t){const i=new MutationObserver(r=>{r.forEach(r=>{if(r.attributeName==="style"){const u=getComputedStyle(r.target).getPropertyValue(n).trim();if(u){const n=parseFloat(u);t(n,i)}}})});i.observe(document.documentElement,{attributes:!0})}observeContentChanges(n,t){const i=new MutationObserver(n=>{n.forEach(n=>{n.type==="childList"&&t(i)})});i.observe(n,{childList:!0})}updateUI(n){this.querySelector(".mud-ex-app-loader-loading-percentage").innerHTML=`${n}%`;this.updateCircleProgress(n);n>=100&&setTimeout(()=>{this.querySelector(".mud-ex-app-loader-circle").classList.add("mud-ex-app-loader-circle-glowing")},2)}updateCircleProgress(n){const t=this.querySelector(".mud-ex-app-loader-circle-progress"),r=t.getAttribute("r"),i=2*Math.PI*r,u=n/100,f=i*(1-u);t.style.strokeDasharray=`${i} ${i}`;t.style.strokeDashoffset=f}}customElements.define("mud-ex-app-loader",MudExAppLoader);